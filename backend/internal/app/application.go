...
func New(ctx context.Context, cfg config.Config, logger *slog.Logger) (*Application, error) {
	...
	if len(cfg.Server.CORS.AllowedOrigins) > 0 {
		...
		cors.AllowedOrigins = cfg.Server.CORS.AllowedOrigins
		cors.AllowCredentials = cfg.Server.CORS.AllowCredentials
	}
	...
	if cfg.RuntimeStore.Driver == "redis" {
		healthCheckers["runtime_store"] = runtimeHealth
	}
	router := httpserver.New(httpserver.Dependencies{Logger: logger, RequestTimeout: cfg.Server.RequestTimeout.Value(), MaxBodyBytes: cfg.Server.MaxBodyBytes, TrustedProxies: cfg.Server.TrustedProxies, ConcurrencyGate: inferenceConcurrency, SecureCookies: cfg.Auth.SecureCookies, SwaggerEnabled: cfg.Server.SwaggerEnabled, PublicAPIBaseURL: cfg.Frontend.EffectivePublicAPIBaseURL(), FrontendStaticPath: cfg.Frontend.StaticPath, CORSConfig: cors, BodyLogConfig: httpmiddleware.BodyLogConfig{Enabled: cfg.Server.BodyLog.Enabled, MaxBodyBytes: cfg.Server.BodyLog.MaxBodyBytes, Logger: logger}, HealthCheckers: healthCheckers, Readiness: readiness, TrafficReady: startup.acceptsTraffic, AdminAuth: adminService, Accounts: accountService, AccountSync: accountSyncService, Models: modelService, ClientKeys: clientKeyService, Audits: auditService, Dashboard: dashboardService, Gateway: gatewayService, Media: mediaService, Settings: settingsService, Egress: egressService, QualityGuardStatePath: qualityGuardPath("state.json"), QualityGuardConfigPath: qualityGuardPath("runtime-config.json"), QualityGuardToken: qualityGuardToken, QualityGuardProbe: qualityGuardProbe, Updates: updateService})
