...
func (a *Adapter) ParseImportedCredentials(data []byte) ([]provider.CredentialSeed, error) {
	...
	if trimmed == "" {
		return nil, fmt.Errorf("Tiada akaun Grok Web dalam fail akaun")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("Huraian JSON akaun Grok Web: %w", err)
	}
	if len(entries) == 0 {
		return nil, fmt.Errorf("Tiada akaun Grok Web dalam fail akaun")
	}
	...
	for index, entry := range entries {
		...
		if tier != account.WebTierAuto && tier != account.WebTierBasic && tier != account.WebTierSuper && tier != account.WebTierHeavy {
			return nil, fmt.Errorf("Tier akaun ke-%d tidak sah", index+1)
		}
