...
func TestConvertResponsesStreamMarksEncryptedChatReasoningEvidence(t *testing.T) {
	...
	// Local reasoning_opaque replay (UPDATE.md patch 8) intentionally carries the
	// encrypted blob to Chat clients so reasoning survives across turns. The
	// anti-leak contract narrows to: no raw encrypted_content key, and the blob
	// surfaces exactly once via the reasoning_opaque channel.
	if strings.Contains(text, "encrypted_content") {
		t.Fatalf("raw encrypted reasoning key leaked into Chat JSON: %s", text)
	}
	if strings.Count(text, "signature") != 1 || strings.Count(text, `"reasoning_opaque":"signature"`) != 1 {
		t.Fatalf("encrypted reasoning must only surface via reasoning_opaque: %s", text)
	}
}
